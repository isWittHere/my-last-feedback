...
const MATCH_TITLES = new Set(["你好", "问候", "打招呼", "Greeting"]);
...
if (help) {
  console.log(`Usage:
  node scripts/cleanup-opencode-greeting-title-tests.mjs
  node scripts/cleanup-opencode-greeting-title-tests.mjs --apply
...
function isMatchingSession(session) {
  return typeof session?.title === "string" && MATCH_TITLES.has(session.title.trim());
}
...
try {
  ...
  console.log(`Found ${matches.length} session(s) with exact title in ${JSON.stringify([...MATCH_TITLES])}.`);
