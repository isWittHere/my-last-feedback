...
const DIFF_PANEL_ROWS = 3;
const DIFF_PANEL_MIN_SQUARE_SIZE = 9;
...
export function AgentDiffIndicator({ session }: { session: AgentSession }) {
  ...
  const panelSquaresRef = useRef<HTMLDivElement | null>(null);
  const [panelColumnCount, setPanelColumnCount] = useState(16);
  ...
  const panelSegments = buildDiffSegments(compactAdditions, compactDeletions, panelColumnCount * DIFF_PANEL_ROWS);

  useEffect(() => {
    const element = panelSquaresRef.current;
    if (!element) return;
    const updateColumnCount = () => {
      const styles = window.getComputedStyle(element);
      const paddingX = Number.parseFloat(styles.paddingLeft || "0") + Number.parseFloat(styles.paddingRight || "0");
      const gap = Number.parseFloat(styles.columnGap || styles.gap || "3") || 3;
      const availableWidth = Math.max(0, element.clientWidth - paddingX);
      const nextColumnCount = Math.max(1, Math.floor((availableWidth + gap) / (DIFF_PANEL_MIN_SQUARE_SIZE + gap)));
      setPanelColumnCount((current) => current === nextColumnCount ? current : nextColumnCount);
    };
    updateColumnCount();
  ...
  return (
    <div className="agent-diff-indicator-wrap" style={diffVisualStyle}>
      ...
      <div className="agent-diff-popover" data-preview-overlay>
        <div className="agent-diff-popover-scroll">
          ...
          <div ref={panelSquaresRef} className="agent-diff-panel-squares" style={{ "--agent-diff-panel-columns": panelColumnCount } as CSSProperties} aria-label={t("agentConsole.diffHeatmap", "Diff blocks")}>
